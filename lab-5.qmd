...
ggplot(births14_habitgiven, aes(x = habit, y = weight, fill = habit)) +
  ...
  theme_minimal() +
  labs(
    title = "Baby Weights by Mother's Smoking Habit",
    x = "Smoking Habit",
    y = "Baby Weight (lbs)"
  )
```

Based on the boxplot, mothers who are nonsmokers tend to have babies with a slightly higher median weight compared to mothers who smoke. Both distributions appear to have a similar spread and include a few outliers on the lower end (babies with lower birth weights), but the overall distribution for nonsmokers is slightly higher.

c.  Now, fit a linear model that predicts `weight` from `habit`. Provide the tidy summary output below.

```{r}
weight_habit_fit <- linear_reg() |> 
  fit(weight ~ habit, data = births14_habitgiven)

tidy(weight_habit_fit)
```
...
d.  Write the estimated least squares regression line below using proper notation.
    ...
    :::
$$ \widehat{weight} = 7.2698731 + -0.5926801 \times smoker $$
Smoker equals 1 if mother smokes, 0 if not
...
```{r}
weight_mage_fit <- linear_reg() |> 
  fit(weight ~ mage, data = births14)

tidy(weight_mage_fit)
```
...
R determines the regression line using least squares regression. This method fits the line by minimizing the sum of the squared residuals, meaning it finds the line that makes the total squared vertical distances between the observed data points and the predicted values on the line as small as possible.
...
The intercept is saying that for a mother who is 0 years old, we would expect her baby's weight to be 6.7929667 pounds on average. This intercept is not meaningful in this context because a mother cannot be 0 years old.

d.  Interpret the slope in the context of the data and the research question.

For each additional year older a mother is, we expect her baby's weight to increase by 0.0142428 pounds, on average. So a slight positive correlation between a mothers age and baby weight at birth.
...
The response variable is parsim, which is the measure of parasite similarity (the proportion of parasites shared between species).
...
ggplot(parasites, aes(x = divergence_time, y = parsim)) +
  ...
  labs(
    ...
    y = "Parasite Similarity (Proportion)"
  )
```

d.  Use the visualization to describe the relationship between the two variables.

There is a negative, non-linear relationship between divergence time and parasite similarity. As divergence time increases, the proportion of shared parasites quickly drops and eventually approaches zero. Most of the data is clustered at lower divergence times, making the relationship appear L-shaped or heavily skewed.
...
tidy(dt_fit)
```
$$ \widehat{parsim} = 0.379486678 + -0.006531791 \times divergence\_time $$
b.  Interpret the slope and the intercept in the context of the data.

Two species that diverged 0 million years ago (meaning they are exactly the same species) are expected to share a proportion of 0.379486678 of their parasites, on average.
Slope: For each additional million years of divergence time between two species, we expect their parasite similarity to decrease by -0.006531791, on average.
...
ggplot(parasites, aes(x = divergence_time, y = parsim)) +
  ...
  theme_minimal() +
  labs(
    title = "Parasite Similarity vs. Divergence Time with Linear Model",
    x = "Divergence Time (Millions of Years)",
    y = "Parasite Similarity (Proportion)"
  )
```
...
Because it is a straight line applied to a curved relationship, the regression line will eventually drop below zero on the y-axis for very large divergence times. Predicting a negative proportion of parasite similarity is impossible (it doesn't make sense for the porportion to be negative), as parsim is strictly bounded between 0 and 1.
...
ggplot(parasites, aes(x = divergence_time, y = transformed_parsim)) +
  ...
  labs(
    ...
    x = "Divergence Time (Millions of Years)",
    y = "Transformed Parasite Similarity (Log-Odds)"
  )
```

c.  Write a 1-2 sentence description of what you observe in the visualization.

By transforming the response variable, the relationship now appears much more linear and spread out along the y-axis. The linear regression line fits the data significantly better, and because it is modeling log-odds, predictions are no longer constrained by the 0 and 1 boundaries.
...
```{r}
dt_model <- linear_reg() |> fit(transformed_parsim ~ divergence_time, data = parasites)
dist_model <- linear_reg() |> fit(transformed_parsim ~ distance, data = parasites)
BM_model <- linear_reg() |> fit(transformed_parsim ~ BMdiff, data = parasites)
prec_model <- linear_reg() |> fit(transformed_parsim ~ precdiff, data = parasites)
```
...
Slope for divergence_time: -0.09066296
Slope for distance: -0.0002739886	
Slope for BMdiff: -5.044374e-06
Slope for precdiff: -0.006012838
...
No, it is not useful to compare the raw slopes. The explanatory variables are all measured in completely different units and scales (e.g., millions of years, kilometers, grams, millimeters). A 1-unit change in divergence time represents a massive evolutionary shift, whereas a 1-unit change in distance is just a single kilometer. So, the slope magnitudes are not directly comparable.
